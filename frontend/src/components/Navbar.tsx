...
const Navbar = (): React.JSX.Element => {
	...
	return (
		<div className='relative w-full h-32 flex justify-center items-center py-4 bg-white xl:flex-col xl:h-fit xl:py-8 xl:max-h-70'>
			<a
				href='/
			'>
				<img src='/assets/logo_header.png' alt='Novo Header' className='h-24 w-auto xl:h-36 xl:mb-4' />
			</a>
			<div
				...
				<Link onClick={toggleIsNavOpen} to={"/shop"}>
					Shop
				</Link>
				<Link onClick={toggleIsNavOpen} to={"/about"}>
					About
				</Link>
				<Link onClick={toggleIsNavOpen} to={"/gallery"}>
					Gallery
				</Link>
				<Link onClick={toggleIsNavOpen} to={"/contact"}>
					Contact
				</Link>
			</div>
			...
			<div
				id='navIcon'
				className='absolute w-6 h-4 top-12 left-12  [&_span]:bg-brown [&_span]:w-full [&_span]:h-0.5 [&_span]:rounded [&_span]:origin-center overflow-clip xl:hidden'
